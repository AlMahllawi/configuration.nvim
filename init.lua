...
-- Open terminal
vim.keymap.set('n', '<leader>ti', function() vim.cmd.term() end, { desc = 'Open [T]erminal [I]ternally' })
      ...
      local servers = {
        ...
        eslint = {
          root_dir = function(fname)
            if #vim.fs.find({ 'biome.json', 'biome.jsonc' }, { upward = true, path = fname }) > 0 then return nil end
            ...
            if root[1] then return vim.fs.dirname(root[1]) end
            ...
            if pkg_json_path then
              ...
              if f then
                ...
                f:close()
                if contents:find '"eslintConfig"' then return vim.fs.dirname(pkg_json_path) end
              end
            end
            return nil
          end,
  ...
  },
  {
    ...
    config = function()
      ...
      if vim.fn.executable(codelldb_path) ~= 1 then
        vim.notify('codelldb binary not found at: ' .. codelldb_path,
          vim.log.levels.WARN)
      end
        ...
        formatters = {
          ...
          prettier = {
            condition = function(self, ctx)
              return vim.fs.find({ '.prettierrc', '.prettierrc.json' },
